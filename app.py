...
import re
from flow import Flowshop, RandomFlowshop
...
def ganttfig_to_json(fig):
    """
    Biến biểu đồ gantt thành json bằng bộ mã hóa json plotly 
    Attributes:
        fig: Biểu đồ gantt chart

    Returns:
        str: File json biểu diễn bằng một gannt 
    """
...
def jobs_to_gantt_fig(scheduled_jobs, nb_machines, nb_jobs):
    """
    Trả lại một biểu đồ plotly cho một biểu đồ gantt được làm từ việc làm 
    Attributes:
        scheduled_jobs (list): Danh sách các jobs trong tổ m1, m2, ..., mn
        nb_machines (int): Số lượng tổ làm việc
        nb_jobs (int): Số lượng công việc

    Returns:
        Plotly.figure: Tạo ra biểu đồ gantt chart
    """
...
def random_johnson(nb_machines, nb_jobs):
    ...
    fig = jobs_to_gantt_fig(jobs, random_problem.get_number_machines(), random_problem.get_number_jobs())
